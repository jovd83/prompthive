...
export class SearchDiscoveryPage {
    ...
    constructor(page: Page) {
        ...
        this.commandPaletteInput = page.getByTestId('kbar-animator').locator('input');
    ...
    async triggerCommandPalette() {
        // Reset state if already open
        const animator = this.page.getByTestId('kbar-animator');
        if (await animator.isVisible()) {
            await this.page.keyboard.press('Escape');
            await expect(animator).toBeHidden({ timeout: 5000 });
        }

        // 1. Try Keyboard (Standard for KBar)
        await this.page.keyboard.press('Control+k');

        // 2. Check if it worked
        const input = animator.locator('input, textarea').first();
        try {
            await input.waitFor({ state: 'visible', timeout: 3000 });
        } catch (e) {
            // 3. Fallback: UI trigger
            const trigger = this.page.getByTestId('command-palette-search-trigger');

...
erDiagram
    ...
    PromptVersion {
        ...
        string changelog
        string createdById FK
        string agentUsage "Optional AI instructions"
        string agentSkillIds "JSON array of skill IDs"
    }
...
- **PromptVersion**: Immutable snapshot of the prompt content.
  ...
  - `agentUsage`: Optional textarea for agent-specific instructions.
  - `agentSkillIds`: JSON array of IDs referencing prompts of type `AGENT_SKILL`.
...
Utility for generating professional Markdown exports of prompts and skills.

- **Logic**: Handles the construction of the Markdown string, including specific headings for Agent Integration (Agents/Agentskills).
- **Download**: Provides `downloadStringAsFile` for client-side blob generation.
...
MyPromptHive includes deep integration for AI Agent workflows, allowing users to treat prompts as "Skills" and link them together. This bridge enables seamless context handover to external agentic systems.

### 1. Agent Skills Management

Prompts can be categorized with `itemType: 'AGENT_SKILL'`. These items represent specific capabilities an AI agent can perform.

- **Badges**: Skills are visually distinguished with an "🤖 Skill" badge.
- **Context Linking**: Standard prompts can link to multiple Agent Skills. These links are stored in `PromptVersion.agentSkillIds` as a JSON array of technical references.
- **Instruction Injection**: The `agentUsage` field in `PromptVersion` allows for specific instructions on *how* an agent should utilize the skill or prompt.

### 2. Advanced Copy (Context Blocks)

The core value proposition of Agent Integration is the **Advanced Copy** feature in `components/PromptDetail.tsx`.

- **Popover Interface**: When copying a prompt, a popover allows users to toggle specific "Context Blocks".
- **Dynamic Construction**: The clipboard content is built from:
  - **Prompt Content**: The actual text of the prompt.
  - **Agent Usage**: Instructions on agent behavior.
  - **Required Skills**: Descriptions and metadata for all linked Agent Skills.
  - **Source of Truth (SOT) Policy**: Injection of project-specific grounding rules.
- **User Handoff**: Optional section for human-to-agent task delegation.

### 3. Group Skill Import

MyPromptHive facilitates rapid ecosystem building via **Group Skill Import** (`actions/skills.ts`).

- **Workflow**: Users provide a list of GitHub repository URLs.
- **Automation**: The service fetches repository metadata via the GitHub API (name, description), generates an `installCommand` (e.g., `npx skills add ...`), and creates a new `AGENT_SKILL` entry.
- **Organization**: Imported skills are automatically grouped into a timestamped collection (e.g., `20260422_Skillimport`).

## 🧩 Recursive Logic

### Recursive Counts

The application computes recursive counts for collections (showing total prompts in a folder _and_ its sub-folders).
...
- **Database**: We store the _relative path_ string (e.g., `/uploads/avatar-123.jpg`) in the database models, never the binary blob itself.
...
- **Decision:**
  ...
  - **Why Chosen:** Industry standard for salt+hashing passwords. Native implementation for maximum security and performance.
...
graph TD
    ...
    subgraph "Agent Integration"
        Services --> Skills[Agent Skill Service]
        Services --> GitHub[GitHub API / Import]
    end
    ...
    subgraph "Presentation Layer (UI)"
        ...
        Components --> Tailwind[Tailwind CSS UI]
        Components --> AdvancedCopy[Advanced Copy Popover]
    end
```

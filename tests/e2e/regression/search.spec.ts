...
test.describe('Search and Discovery', () => {
  ...
  test('Command Palette: Theme Toggle, Navigation, and Quick Actions', async ({ page }) => {
    test.setTimeout(180000); // High timeout for slow environments
    const searchPage = new SearchDiscoveryPage(page);
    await searchPage.goto();
    ...
    await expect(page.getByTestId('kbar-animator')).toBeHidden({ timeout: 5000 });
    await searchPage.triggerCommandPalette();
    await expect(searchPage.commandPaletteInput).toBeVisible({ timeout: 20000 });
    await searchPage.commandPaletteInput.fill('Theme');
    await page.getByTestId('kbar-action-theme').click();
    await expect(page.getByTestId('kbar-animator')).toBeHidden({ timeout: 20000 });
    ...
    await expect(page.getByTestId('kbar-animator')).toBeHidden({ timeout: 10000 });
    await searchPage.triggerCommandPalette();
    await searchPage.commandPaletteInput.press('Control+a');
    await searchPage.commandPaletteInput.press('Backspace');
    await page.keyboard.type('Settings', { delay: 50 });
    const settingsAction = page.getByTestId('kbar-action-settings');
    await expect(settingsAction).toBeVisible({ timeout: 20000 });
    await settingsAction.click();
    await expect(page.getByTestId('kbar-animator')).toBeHidden({ timeout: 20000 });
    await page.waitForURL(/\/settings$/, { timeout: 30000 });
    await page.waitForLoadState('networkidle');
    await expect(page.getByRole('heading', { level: 1, name: /Settings|Paramètres/i })).toBeVisible({ timeout: 15000 });
    ...
    const helpAction = page.getByTestId('kbar-action-help');
    await expect(helpAction).toBeVisible({ timeout: 20000 });
    await helpAction.click();
    await expect(page.getByTestId('kbar-animator')).toBeHidden({ timeout: 20000 });
    await page.waitForURL(/\/help$/, { timeout: 30000 });
    await page.waitForLoadState('networkidle');
    await expect(page.getByRole('heading', { level: 1, name: /Help|Aide|Assistance/i })).toBeVisible({ timeout: 15000 });
    ...
    await searchPage.commandPaletteInput.press('Control+a');
    await searchPage.commandPaletteInput.press('Backspace');
    await page.keyboard.type('New Prompt', { delay: 50 }); // Using "New Prompt" to match keywords/name better
    const createPromptAction = page.getByTestId('kbar-action-create-prompt');
    await expect(createPromptAction).toBeVisible({ timeout: 20000 });
    await createPromptAction.click();
    await expect(page.getByTestId('kbar-animator')).toBeHidden({ timeout: 20000 });
    await page.waitForURL(/\/prompts\/new$/, { timeout: 30000 });
    await page.waitForLoadState('networkidle');
    await expect(page.getByRole('heading', { level: 1, name: /New Prompt|Nouveau prompt/i })).toBeVisible({ timeout: 15000 });
  });
